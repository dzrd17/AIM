...
.logo {
  ...
  font-size: var(--logo-size, var(--text-lg));
...
.display {
  font-size: var(--display-size, clamp(32px, 6vw, 55px));
  letter-spacing: -0.04em;
...
.ds-table { width: 100%; border-collapse: collapse; font-size: var(--text-sm); margin-top: var(--space-4); }
.ds-table th, .ds-table td { padding: var(--space-3); text-align: left; border-bottom: 2px solid var(--border); }
.ds-table th { background: var(--bg); font-family: var(--font-mono); font-weight: 700; text-transform: uppercase; font-size: var(--text-xs); }
...
.stat strong {
  ...
  white-space: nowrap;
  font-family: var(--font-display);
  font-size: var(--stat-size, clamp(var(--text-2xl), 5vw, 55px));
  letter-spacing: -0.08em;
  line-height: .9;
  overflow: hidden;
}
.stat strong.word { font-size: var(--stat-word-size, clamp(var(--text-xl), 4vw, 50px)); }
.stat span {
  display: block;
...
function collectTools() {
  ...
  return bundlePaths.map((bundlePath) => {
    ...
    const folder = path.basename(bundleDir).replace(/\.pushbutton$/i, "");
    const scriptPath = path.join(bundleDir, "script.py");
    ...
    // Media detection
    const mediaFiles = fs.readdirSync(bundleDir).filter(f => 
      f.toLowerCase().endsWith(".gif") || 
      f.toLowerCase().endsWith(".mp4") || 
      f.toLowerCase().endsWith(".webm")
    );
    // Sort to prioritize .gif if requested specifically
    mediaFiles.sort((a, b) => {
      if (a.toLowerCase().endsWith(".gif") && !b.toLowerCase().endsWith(".gif")) return -1;
      if (!a.toLowerCase().endsWith(".gif") && b.toLowerCase().endsWith(".gif")) return 1;
      return 0;
    });
    let mediaPath = "";
    if (mediaFiles.length > 0) {
      const sourceMedia = path.join(bundleDir, mediaFiles[0]);
      const ext = path.extname(mediaFiles[0]);
      const targetMediaName = `${slug}${ext}`;
      fs.copyFileSync(sourceMedia, path.join(mediaDir, targetMediaName));
      mediaPath = `../media/${targetMediaName}`;
    }
...
function renderHeader(prefix = "") {
  return `<header class="topnav" data-od-id="topnav">
    <div class="container topnav-inner">
      <a class="logo" href="${prefix}index.html" aria-label="DH Standard 홈" style="--logo-size: var(--text-lg);">
...
function getLatestUpdateDate(tools) {
  let latest = 0;
  for (const tool of tools) {
    try {
      if (fs.existsSync(tool.scriptPath)) {
        const mtime = fs.statSync(tool.scriptPath).mtimeMs;
        if (mtime > latest) latest = mtime;
      }
    } catch (e) {}
  }
  const date = latest ? new Date(latest) : new Date();
  return `${date.getFullYear()}.${String(date.getMonth() + 1).padStart(2, "0")} 기준`;
}

function indexPage(tools) {
  const counts = categoryCounts(tools);
  const formattedDate = getLatestUpdateDate(tools);
    ...
    body: `    <section class="hero" data-od-id="hero">
      <div class="container hero-grid">
        <div>
          ...
          <h1 class="display" style="--display-size: clamp(32px, 6vw, 55px);">반복 작업을<br />버튼으로 끝낸다.</h1>
          <p class="lead">DH_Standard.extension의 현재 배포 스크립트를 탐색하고, 작동에 필요한 패밀리나 매개변수 정보를 확인합니다.</p>
        ...
        <div class="stat-board" aria-label="스크립트 현황">
          <div class="stat"><strong style="--stat-size: clamp(var(--text-2xl), 5vw, 55px)">${tools.length}</strong><span>현재 실행 스크립트</span></div>
          <div class="stat"><strong style="--stat-size: clamp(var(--text-2xl), 5vw, 55px)">03</strong><span>리본 탭</span></div>
          <div class="stat"><strong class="word" style="--stat-word-size: clamp(var(--text-xl), 4vw, 42px)">${formattedDate}</strong><span>현재 배포본 기준</span></div>
        </div>
      </div>
    </section>
  ...
  return baseDocument({
    ...
    body: `    <section class="hero" data-od-id="hero">
      <div class="container">
        ...
        <h1 class="display" style="--display-size: clamp(32px, 6vw, 64px);">${meta.label}<br />기능 목록.</h1>
...
function detailPage(tool, tools) {
  ...
  const mediaHtml = tool.mediaPath 
    ? (tool.mediaPath.endsWith(".mp4") || tool.mediaPath.endsWith(".webm")
        ? `<video src="${tool.mediaPath}" controls muted loop style="width: 100%; border: 3px solid var(--border); box-shadow: 8px 8px 0 #000;"></video>`
        : `<img src="${tool.mediaPath}" alt="${escapeHtml(tool.title)} 시연" style="width: 100%; border: 3px solid var(--border); box-shadow: 8px 8px 0 #000;" />`)
    : `<div class="demo-slot">
        <div>
  ...
  return baseDocument({
    ...
    body: `    <section class="hero detail-hero" data-od-id="hero">
      <div class="container">
        ...
        <h1 class="display" style="--display-size: clamp(32px, 6vw, 64px); white-space: nowrap; max-width: none;">${escapeHtml(tool.title)}</h1>
      ...
      <div class="container detail-layout">
        <div class="detail-stack">
          <article class="detail-card">
            <h2>필수 패밀리 및 매개변수</h2>
            <p>이 기능을 작동하는 데 필요한 Revit 요소와 정보입니다.</p>
            <table class="ds-table">
              ...
              <tbody>
                <tr>
                  <td>[이름 입력]</td>
                  <td>[카테고리/타입]</td>
                  <td>기능 작동에 필요한 구체적인 정보 또는 속성 이름을 입력하세요.</td>
                </tr>
              </tbody>
            </table>
          </article>
